...
```bash
curl -X POST https://travel-portal-dev.preview.emergentagent.com/api/expedia/setup \
...
```bash
curl -X POST https://travel-portal-dev.preview.emergentagent.com/api/expedia/flights/search \
  -H "Content-Type: application/json" \
  -d '{
    "origin": "LAX",
    "destination": "JFK",
...
```bash
curl -X POST https://travel-portal-dev.preview.emergentagent.com/api/expedia/cars/search \
  -H "Content-Type: application/json" \
  -d '{
    "pickup_location": "LAX",
...
```bash
curl -X POST https://travel-portal-dev.preview.emergentagent.com/api/expedia/activities/search \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://travel-portal-dev.preview.emergentagent.com/api/expedia/hotels/book \
  -H "Content-Type: application/json" \
  -d '{
    "property_id": "123456",
    "room_id": "room_1",
...
async def search_expedia_hotels(checkin: date, checkout: date, adults: int = 2):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "https://travel-portal-dev.preview.emergentagent.com/api/expedia/hotels/search",
            json=search_params,
            timeout=30.0
        )
...
async def safe_expedia_request(endpoint: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    base_url = "https://travel-portal-dev.preview.emergentagent.com/api/expedia"
...
def get_expedia_config() -> ExpediaConfig:
    ...
    return ExpediaConfig(
        base_url=os.getenv('EXPEDIA_BASE_URL', 'https://travel-portal-dev.preview.emergentagent.com'),
