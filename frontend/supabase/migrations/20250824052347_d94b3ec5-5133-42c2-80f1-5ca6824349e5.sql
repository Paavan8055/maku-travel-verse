-- Phase 3C: Final Production Infrastructure Setup

-- Create API health monitoring table if not exists (fix the missing table issue)
...
-- Create policies
CREATE POLICY "Admins can view API health monitoring" ON public.api_health_monitoring
FOR SELECT USING (is_secure_admin(auth.uid()));

CREATE POLICY "Service role can manage API health monitoring" ON public.api_health_monitoring
...
-- Create the missing log_api_health function
