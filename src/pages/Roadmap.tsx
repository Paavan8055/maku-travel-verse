...
const categoryConfig = {
  ...
  community: {
    icon: Users,
    color: 'from-travel-coral to-travel-sunset',
    label: 'Community'
  },
  ai: {
    icon: Brain,
    color: 'from-travel-gold to-travel-sunset',
    label: 'AI & Tech'
  },
  mobile: {
    icon: Globe,
    color: 'from-travel-forest to-travel-ocean',
    label: 'Mobile'
  },
  partnerships: {
    icon: Building2,
    color: 'from-travel-sky to-travel-coral',
    label: 'Partnerships'
  },
  blockchain: {
    icon: Shield,
...
const statusConfig = {
  ...
  'in-progress': {
    ...
    bg: 'bg-travel-gold/10',
    label: 'In Progress'
  },
  planned: {
    icon: Target,
    color: 'text-travel-ocean',
    bg: 'bg-travel-ocean/10',
    label: 'Planned'
  },
  future: {
    icon: Compass,
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-16">
          ...
          <div className="flex items-center justify-center gap-6 mb-12">
            <Button onClick={() => setIsPlaying(!isPlaying)} size="lg" className="bg-gradient-to-r from-travel-coral to-travel-sunset hover:shadow-floating animate-scale-in">
        ...
        <div className="relative">
          ...
          <div className="space-y-12">
            ...
            return <div key={item.id} className={`relative transition-all duration-1000 ${isActive ? 'scale-105' : 'scale-100'}`} onClick={() => setCurrentStep(index)}>
                  {/* Animated Timeline Dot */}
                  <div className={`absolute left-5 top-8 w-6 h-6 rounded-full border-4 shadow-lg hidden md:block z-10 cursor-pointer transition-all duration-500 ${isActive ? 'bg-travel-gold border-travel-gold shadow-travel-gold/50 animate-pulse-soft scale-125' : isCompleted ? 'bg-green-500 border-green-500 shadow-green-500/30' : 'bg-white border-travel-ocean shadow-travel-ocean/20'}`}>
                    {isActive && <div className="absolute inset-0 rounded-full bg-travel-gold animate-ping opacity-75"></div>}
                  </div>
                  
                  <Card className={`md:ml-20 transition-all duration-700 cursor-pointer group ${isActive ? 'shadow-floating border-travel-gold bg-gradient-to-r from-travel-gold/5 to-travel-sunset/5' : 'hover:shadow-floating hover:border-travel-coral/50'}`}>
                    <CardHeader>
                      <div className="flex items-start justify-between flex-wrap gap-4">
                        <div className="flex items-start gap-6 flex-1">
                          <div className={`p-4 rounded-2xl bg-gradient-to-r ${category.color} transition-transform duration-300 ${isActive ? 'animate-float' : 'group-hover:scale-110'}`}>
                            <category.icon className="h-8 w-8 text-white" />
                          </div>
                          
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                              <CardTitle className={`text-2xl transition-colors duration-300 ${isActive ? 'text-travel-gold' : 'group-hover:text-primary'}`}>
                                {item.title}
                              </CardTitle>
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            {isActive && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-slideIn"></div>}
                          </div>
                        </div>

                        {/* Impact Highlight */}
                        <div className={`p-4 rounded-xl border transition-all duration-300 ${isActive ? 'bg-gradient-to-r from-travel-gold/10 to-travel-sunset/10 border-travel-gold/30' : 'bg-gradient-to-r from-travel-gold/5 to-travel-sunset/5 border-travel-gold/20'}`}>
                          <div className="flex items-center gap-2 mb-2">
                            <Star className={`h-5 w-5 transition-colors duration-300 ${isActive ? 'text-travel-gold animate-pulse-soft' : 'text-travel-gold'}`} />
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {item.features.map((feature, idx) => <Badge key={idx} variant="outline" className={`text-sm p-2 justify-start transition-all duration-300 ${isActive ? 'hover:bg-travel-gold/10 hover:border-travel-gold' : ''}`}>
