...
import { Send, Bot, Loader2, AlertCircle, CheckCircle } from 'lucide-react';
...
export const UniversalBotInterface: React.FC<UniversalBotInterfaceProps> = ({
  dashboardType,
  className = '',
}) => {
  ...
  const handleSendMessage = async () => {
    if (!inputValue.trim() || isExecutingCommand) return;
    ...
    try {
      // For admin dashboard, execute command directly
      if (dashboardType === 'admin') {
        // Determine command type from user input
        const commandType = userMessage.toLowerCase().includes('analyze') ? 'analysis' :
                          userMessage.toLowerCase().includes('optimize') ? 'optimization' :
                          userMessage.toLowerCase().includes('control') ? 'control' : 'query';

        const response = await executeAdminCommand(
          userMessage,
          commandType,
          [], // We'll let the backend determine target bots
          {}
        );
      ...
      <CardContent className="flex-1 flex flex-col p-4">
          ...
          <div className="space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-muted-foreground py-8">
                ...
                <p>Start a conversation with the Master Bot</p>
                <p className="text-sm">
                  {dashboardType === 'admin' 
                    ? 'Use natural language to control and analyze your bot network'
                    : `Get personalized assistance for ${dashboardType} tasks`
                  }
                </p>
              </div>
            )}
        ...
        <div className="flex gap-2 mt-4">
          <Input
            ...
            placeholder={
              dashboardType === 'admin'
                ? 'Enter command (e.g., "Analyze booking performance" or "Optimize pricing")'
                : 'Ask me anything about your travel needs...'
            }
            onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
            disabled={isExecutingCommand || isStreaming}
            className="flex-1"
          />
          <Button
            onClick={handleSendMessage}
            disabled={!inputValue.trim() || isExecutingCommand || isStreaming}
            size="icon"
          >
            {isExecutingCommand || isStreaming ? (
