...
import { Plane, Hotel, Calendar, Settings } from 'lucide-react';
...
interface AgenticInlinePanelProps {
  userVertical: 'Family' | 'Solo' | 'Pet' | 'Spiritual';
}

const AgenticInlinePanel: React.FC<AgenticInlinePanelProps> = ({ userVertical }) => {
  ...
  return (
    <section aria-labelledby="agentic-inline-title" className="container mx-auto px-4">
      <div className="rounded-3xl border border-border bg-card shadow-floating overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border bg-gradient-hero">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur">🤖</div>
            <div>
              <h3 id="agentic-inline-title" className="font-semibold text-white">
                Maku Agent
              </h3>
              <p className="text-sm text-white/80">Your AI travel assistant</p>
            </div>
          </div>
          {/* No close button for inline version */}
        </div>
...
export default AgenticInlinePanel;
