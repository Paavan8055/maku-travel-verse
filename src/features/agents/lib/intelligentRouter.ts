...
export class IntelligentRouter {
  ...
  private async getAvailableAgents(type: 'gpt_bot' | 'internal_agent') {
    if (type === 'gpt_bot') {
  ...
  private async selectBestAgent(agents: any[], context: TaskContext, type: 'gpt_bot' | 'internal_agent'): Promise<RoutingDecision> {
    if (agents.length === 0) {
    ...
    return {
      agentId: bestAgent.agent.id || bestAgent.agent.agent_id,
      agentType: type,
  ...
  private async scoreAgent(agent: any, context: TaskContext, type: 'gpt_bot' | 'internal_agent') {
    let score = 0;
    let reasoning = [];

    // Performance score (40%)
    const successRate = type === 'gpt_bot' ? agent.success_rate || 0.95 : 0.98;
    ...
    // Load balancing (20%)
    const currentLoad = await this.getAgentLoad(
      type === 'gpt_bot' ? agent.bot_id : agent.agent_id
    );
  ...
  private calculateCapabilityMatch(capabilities: any[], intent: string): number {
    if (!capabilities.length) return 0.5; // Default moderate match
    
    const intentWords = intent.toLowerCase().split(' ');
    let maxMatch = 0;

    capabilities.forEach(cap => {
      const capWords = (cap.name || '').toLowerCase().split(' ');
  ...
  private calculateCostEfficiency(agent: any, context: TaskContext): number {
    ...
    const baseCost = agent.cost_per_request || 0.02;
  ...
  private estimateCost(model: string, context: TaskContext): number {
    ...
    const baseCost = baseCosts[model] || 0.02;
  ...
  private estimateDuration(agent: any, context: TaskContext): number {
    const baseTime = agent.average_response_time || 2000; // milliseconds
  ...
  private async defaultRouting(context: TaskContext): Promise<RoutingDecision> {
    ...
    return {
      agentId: agents[0].id || agents[0].agent_id,
