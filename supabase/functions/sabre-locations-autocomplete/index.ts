...
function transformSabreLocationResponse(sabreResults: any[]): any[] {
  ...
  try {
    for (const result of sabreResults) {
      ...
      if (result.Locations) {
        const locations_data = Array.isArray(result.Locations)
          ? result.Locations
          : [result.Locations];
        ...
        for (const location of locations_data) {
          locations.push({
            ...
            coordinates: location.Position || location.GeoCoordinates
              ? {
                  latitude: parseFloat(
                    location.Position?.Latitude || location.GeoCoordinates?.Latitude || '0'
                  ),
                  longitude: parseFloat(
                    location.Position?.Longitude || location.GeoCoordinates?.Longitude || '0'
                  ),
                }
              : null,
