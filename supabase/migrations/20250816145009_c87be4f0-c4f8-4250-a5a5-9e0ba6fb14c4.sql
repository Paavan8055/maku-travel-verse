-- Fix remaining RLS policy issues with correct column references

-- Enable RLS on partner_onboarding_payments table if not already enabled
...
-- Enable RLS on guest_booking_tokens table if not already enabled
ALTER TABLE public.guest_booking_tokens ENABLE ROW LEVEL SECURITY;

-- Create policy for guest_booking_tokens - only allow token verification with proper email match
...
USING (
  EXISTS (
    ...
    AND b.booking_data->'customerInfo'->>'email' IS NOT NULL
    AND email_hash = encode(digest((b.booking_data->'customerInfo'->>'email') || 'booking_salt_' || b.id::text, 'sha256'), 'hex')
  )
);

-- Enable RLS on booking_access_audit table if not already enabled
...
-- Enable RLS on ai_training_bookings table if not already enabled
