...
-- Create updated_at trigger for enhanced_agent_memory
CREATE OR REPLACE FUNCTION public.update_enhanced_agent_memory_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER update_enhanced_agent_memory_updated_at
  BEFORE UPDATE ON public.enhanced_agent_memory
  FOR EACH ROW
  EXECUTE FUNCTION public.update_enhanced_agent_memory_updated_at();