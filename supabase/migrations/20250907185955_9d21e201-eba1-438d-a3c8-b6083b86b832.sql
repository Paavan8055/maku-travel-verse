...
ALTER TABLE public.agent_management DROP CONSTRAINT IF EXISTS valid_tier_name;

-- Add new constraints allowing tier 0 and management tier_name
ALTER TABLE public.agent_management ADD CONSTRAINT valid_tier CHECK (tier >= 0 AND tier <= 4);
ALTER TABLE public.agent_management ADD CONSTRAINT valid_tier_name CHECK (tier_name = ANY (ARRAY['executive'::text, 'management'::text, 'operational'::text, 'coordination'::text, 'specialist'::text, 'support'::text]));
