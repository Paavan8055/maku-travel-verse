...
-- Remove public read access to provider_configs
...
-- Fix OTP expiry settings (reduce from 24 hours to 10 minutes for security)
UPDATE auth.config 
SET config_value = 600  -- 10 minutes in seconds
WHERE config_name = 'otp_expiry_seconds';

-- Ensure secure password settings
UPDATE auth.config 
SET config_value = true
WHERE config_name = 'enable_leaked_password_protection';
